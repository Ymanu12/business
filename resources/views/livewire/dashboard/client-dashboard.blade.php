<div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
    ...
    {{-- Hero --}}
    <section class="relative overflow-hidden rounded-[2.2rem] border border-white/70 bg-[linear-gradient(135deg,_#09111f_0%,_#0f766e_45%,_#f59e0b_140%)] px-6 py-8 text-white shadow-2xl shadow-zinc-900/15 lg:px-8 lg:py-10 dark:border-white/15">
        <div class="absolute -right-16 top-0 h-48 w-48 rounded-full bg-white/10 blur-3xl"></div>
        <div class="absolute bottom-0 left-0 h-40 w-40 rounded-full bg-amber-300/20 blur-3xl"></div>
        ...
        <div class="relative flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <h1 class="mt-4 text-3xl font-black leading-tight sm:text-4xl">
                    @if ($activeOrdersCount > 0)
                        Vous avez {{ $activeOrdersCount }} commande{{ $activeOrdersCount > 1 ? 's' : '' }} en cours.
                    @else
                        Prêt à lancer un nouveau projet ?
                    @endif
                </h1>
                <div class="mt-6 flex flex-wrap gap-3">
                    <a href="{{ route('search') }}" class="inline-flex items-center justify-center rounded-full bg-white px-5 py-3 text-sm font-semibold text-zinc-950 transition hover:bg-amber-300 dark:bg-zinc-800 dark:text-white">
                        Commander un service
                    </a>
                    <a href="{{ route('orders.index') }}" class="inline-flex items-center justify-center rounded-full border border-white/20 bg-white/10 px-5 py-3 text-sm font-semibold text-white transition hover:bg-white/15">
                        Voir mes commandes
                    </a>
                </div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-3 sm:grid-cols-1 sm:gap-4 sm:w-56 sm:shrink-0">
                <article class="rounded-[1.6rem] border border-white/12 bg-white/10 p-4 backdrop-blur text-center">
                    <p class="text-[11px] font-semibold uppercase tracking-[0.22em] text-zinc-100/70">En cours</p>
                    <div class="mt-3 text-3xl font-black">{{ $activeOrdersCount }}</div>
                </article>
                <article class="rounded-[1.6rem] border border-white/12 bg-white/10 p-4 backdrop-blur text-center">
                    <p class="text-[11px] font-semibold uppercase tracking-[0.22em] text-zinc-100/70">À valider</p>
                    <div class="mt-3 text-3xl font-black">{{ $pendingDeliveries }}</div>
                </article>
                <article class="rounded-[1.6rem] border border-white/12 bg-white/10 p-4 backdrop-blur text-center">
                    <p class="text-[11px] font-semibold uppercase tracking-[0.22em] text-zinc-100/70">Escrow</p>
                    <div class="mt-3 text-lg font-black">{{ number_format($escrowTotal, 0, ',', ' ') }}</div>
                </article>
            </div>
        </div>
    </section>
    ...
    <div class="mt-8 grid gap-6 xl:grid-cols-[1fr_320px]">
        ...
        <div class="grid gap-4">
            @if ($activeOrders->isEmpty())
                <div class="rounded-[2rem] border border-zinc-200/80 bg-white/90 p-10 text-center shadow-sm backdrop-blur dark:bg-zinc-800/90 dark:border-zinc-700/60">
                    <p class="text-zinc-400 text-sm">Aucune commande en cours.</p>
                    <a href="{{ route('search') }}" class="mt-4 inline-flex items-center justify-center rounded-full bg-zinc-950 px-5 py-3 text-sm font-semibold text-white transition hover:bg-teal-700">
                        Parcourir les services
                    </a>
                </div>
            @else
                <h2 class="text-base font-black text-zinc-950 dark:text-white">Commandes en cours</h2>
                    ...
                    <a href="{{ route('orders.show', $order->uuid) }}" wire:navigate
                       class="block rounded-[1.75rem] border border-zinc-200/80 bg-white/90 p-5 shadow-sm backdrop-blur transition hover:-translate-y-0.5 hover:border-teal-300 hover:shadow-md dark:bg-zinc-800/90 dark:border-zinc-700/60">
                        <div class="flex items-start justify-between gap-4">
                            <div class="min-w-0">
                                ...
                                <p class="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
                                    Freelance : {{ $order->freelancer->name }}
                        ...
                        <div class="mt-4 flex items-center gap-3">
                            ...
                            <span class="text-xs text-zinc-400">{{ $order->daysLeft() }}j restant{{ $order->daysLeft() !== 1 ? 's' : '' }}</span>
                        </div>
                        ...
                        <div class="mt-3 flex items-center justify-between text-xs text-zinc-500 dark:text-zinc-400">
                            ...
                            @if ($order->status === \App\Enums\OrderStatus::Delivered)
                                <span class="font-semibold text-amber-600">En attente de votre validation</span>
                            @endif
                        </div>
                    </a>
                @endforeach
        ...
        <aside class="grid gap-4 self-start">
            <div class="rounded-[1.75rem] border border-zinc-200/80 bg-white/90 p-5 shadow-sm backdrop-blur dark:bg-zinc-800/90 dark:border-zinc-700/60">
                <p class="text-[11px] font-semibold uppercase tracking-[0.22em] text-zinc-400">Wallet</p>
                <div class="mt-3 text-2xl font-black text-teal-700">{{ $wallet->formattedBalance() }}</div>
                <p class="mt-1 text-xs text-zinc-400">Solde disponible</p>
                <a href="{{ route('wallet') }}" wire:navigate
                   class="mt-4 block rounded-full border border-zinc-200 px-4 py-2.5 text-center text-xs font-semibold text-zinc-700 transition hover:border-teal-300 hover:text-teal-700 dark:text-zinc-300 dark:border-zinc-700">
                    Gérer le wallet
                </a>
            </div>

            <div class="rounded-[1.75rem] border border-zinc-200/80 bg-white/90 p-5 shadow-sm backdrop-blur dark:bg-zinc-800/90 dark:border-zinc-700/60">
                <p class="text-[11px] font-semibold uppercase tracking-[0.22em] text-zinc-400">Actions rapides</p>
